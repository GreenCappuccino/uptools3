...
char *updirUnipenCode2WriterName (char *wcode)
{
	UnipenIdentification unipen_id;

	parse_writercode (wcode,&unipen_id);
	return unipen_identifications[unipen_id.idx].writer_name;
}
...
char *updirGetFilenameFromId (int idx)
{
	static char fname[1024];
	UnipenIdentification *uid;

	uid = &unipen_identifications[idx];
	sprintf (fname,"%s/%s",datasets[uid->dataset].path,uid->path);
	return fname;
}

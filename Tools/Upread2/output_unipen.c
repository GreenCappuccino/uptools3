...
int flush_comments(FILE *fp, tUPEntry *entry, tUPEntry **comments, int ncomments, int prev_count)
{
	int c,count,i;
	...
	for (i=0;i<ncomments;i++) {
		c = comments[i]->Count;
		if (c>prev_count&&c<count) {
			fprintf (fp,"%s\n",comments[i]->Entry);
			fprintf (fp,".COMMENT ORIGINAL ENTRY: [%s]\n",entry->Entry);
		}
	}
	return count;
}
...
{
	int count,i,transform_coord;
	static int streamnr = 0;
	tUPEntry **comments;
	int nblocks, ncomments;
	int idx,nindices, *indices=NULL;
	FILE *fp;

	if (nentries==0)
		return 0;

	if (oinfo->index_file[0]!='\0') {
		if ((fp=fopen(oinfo->index_file,"r"))==NULL) {
			fprintf (stderr,"unable to open index-file '%s'!\n",oinfo->index_file);
			exit(1);
		}
		indices = (int *) malloc (1024*sizeof(int));
		nindices = 0;
		nblocks = 1;
		while (fscanf(fp,"%d",&indices[nindices])==1) {
			nindices += 1;
			if (nindices>=nblocks*1024) {
				indices = (int *) realloc (indices,(nblocks+1)*1024*sizeof(int));
				nblocks += 1;
			}
		}
		fprintf (stderr,"READ %d indices\n",nindices);
		fclose(fp);
	} else {
		nindices = 0;
	}
	count     = entries[0]->Count-3;
	ncomments = 0;
	comments  = NULL;

	if (oinfo->add_comments) {
		if ((ncomments=upGetKeywordId (pUnipen,".COMMENT"))>=0) {
			comments  = pUnipen->Entries[ncomments];
			ncomments = pUnipen->NrOfEntries[ncomments];
		}
	}
	...
	if (nindices>0) {
		for (i=0;i<nindices;i++) {
			idx = indices[i];
			fprintf (stderr,"getting entry %d\n",idx);
			if (idx<0||idx>nentries) {
				fprintf (stderr,"SEEMS FATAL TO ME (only %d entries!)\n",nentries);
				continue;
			}
			if (oinfo->add_comments)
				count = flush_comments(fp_out,entries[idx],comments,ncomments,count);
			fprintf (fp_out,".COMMENT segment originating from '%s' (%d streams)\n"
				, info->files[0],streams[idx]->nstreams);
		...
		for (i=0;i<nentries;i++) {
			if (oinfo->add_comments)
				count = flush_comments(fp_out,entries[i],comments,ncomments,count);
			fprintf (fp_out,".COMMENT segment originating from '%s' (%d streams)\n"

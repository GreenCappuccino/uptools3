...
while {![info complete $pkgIndex2] && ![eof $f]} {
    set line [gets $f]
    append pkgIndex2 "${line}\n"
}
close $f
regexp {package ifneeded ([A-Z][a-z_]+) ([0-9\.]+) } $pkgIndex \
		trash Pkgname version
regexp {package ifneeded ([A-Z][a-z_]+) ([0-9\.]+) } $pkgIndex2 \
		trash Pkgname2 version2
...
if {![catch {set f [open [file join $dir pkgIndex.tcl] r]}]} {
	...
	if [string match "*package ifneeded*" $chunk] {
	    if {![string match "package ifneeded $Pkgname $version *" $chunk] &&
		    ![string match "package ifneeded $Pkgname2 $version2 *" $chunk]} {
		append header $chunk
	    }

...
extern double strtod (const char *,char **); /* not always found on all machines */
...
static char *upAddString (char *_String1_, char *_String2_)
{
	...
	_String1_ = (char *) upChangeMemory (_String1_, strlen (_String1_) + strlen (_String2_) + 1);
...
static int upSetUpIncludePaths (tUPUnipen *pUnipen)
{
	...
	if ((Path=getenv(cUnipenIncludePath))!=NULL ) {
		...
		pUnipen->upIncludePaths = (char **) malloc (sizeof(char *));
		pUnipen->upIncludePaths[0] = (char *) malloc (strlen(include_dummy)+1);
		...
		while (Path[0]!='\0') {
			...
			if (sscanf(Path,"%s",include_dummy)==1) {
				pUnipen->upIncludePaths = (char **) realloc (pUnipen->upIncludePaths,(n+1)*sizeof(char *));
				pUnipen->upIncludePaths[n] = (char *) malloc (strlen(include_dummy)+1);
...
int upAdministrateEntry (tUPUnipen *pUnipen, char *entry, int *new_entrynr, int count)
{
	...
	if (entrynr==0)
		pUnipen->Entries[keyword_id] = (tUPEntry **) malloc (sizeof(tUPEntry *));
	else
		pUnipen->Entries[keyword_id] = (tUPEntry **) realloc
			(pUnipen->Entries[keyword_id],(entrynr+1)*sizeof(tUPEntry *));
	if (count==0)
		pUnipen->entry_sequence = (tUPEntry **) malloc (sizeof(tUPEntry *));
	else
		pUnipen->entry_sequence = (tUPEntry **) realloc (pUnipen->entry_sequence,(count+1)*sizeof(tUPEntry *));
...
upBool upReadNextFile (tUPUnipen *pUnipen, char *fname, int *n, int got_version)
{
	...
	if (pUnipen->nunipens==1)
		pUnipen->file_contents = (char **) calloc (1,sizeof(char *));
	else
		pUnipen->file_contents = (char **) realloc (pUnipen->file_contents,pUnipen->nunipens*sizeof(char *));
...
upBool upNextFile (tUPUnipen *pUnipen, char *fname)
{
	...
	if (upReadNextFile (pUnipen,pUnipen->cur_file_open,&j,0)) {
		...
		/* segments in levels */
		pUnipen->NrOfSegmentsInLevel = (int *) calloc (pUnipen->NrOfLevels,sizeof (int));
		for (i=0;i<mNrOfSegments(pUnipen);i++) {
